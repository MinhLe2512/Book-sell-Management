    ...
    public class clsComment
    {
        public void Insert(int userId, int watchID, DateTime date, string content, int idCommentReplid)
        {
            ...
            if(idCommentReplid==-1)
                sqlCom = new MySqlCommand("insert into comments values(@userid,@watchid,@date,@content,1,null)");
            else
                sqlCom = new MySqlCommand("insert into comments values(@userid,@watchid,@date,@content,1,@idCommentReplied)");
