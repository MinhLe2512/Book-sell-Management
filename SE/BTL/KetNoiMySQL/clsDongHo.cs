...
namespace BTL.KetNoiMySQL
{
    public class clsDongHo
    {
        ...
        {
            MySqlCommand sqlCom = new MySqlCommand("insert into LoaiDongHo values (@CategoryName,@xuatxu, @hangsanxuat,1,@th)");
            sqlCom.CommandType = CommandType.Text;
            ...
            string chatlieuday,double sizeDay,string chatlieuvo,double duongkinhvo,double timebaohanh, bool active)
        {
            ...
            if (active == true)
            {
                sqlCom = new MySqlCommand("insert into ChiTietDongHo values(@typeDHid,@code,@price,@sale,@urlPicture,' '" +
