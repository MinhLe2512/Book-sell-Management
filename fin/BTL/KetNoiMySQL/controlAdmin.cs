...
namespace BTL.KetNoiMySQL
{
    public class controlAdmin
    {
        public void Insert(string adminName, int gioiTinh, string image, string password, string address,string phone,string email,string acc)
        {
            ...
            if (gioiTinh == 1)
            {
                sqlCom = new MySqlCommand("insert into users(userName, gioiTinh, urlAvatarPicture, password, address, phone, email, account, ngaySinh, isAdmin) values(@name,1,@image,@password,@address,@phone,@email,@acc,null,1)");
            }
            else
            {
                sqlCom = new MySqlCommand("insert into users(userName, gioiTinh, urlAvatarPicture, password, address, phone, email, account, ngaySinh, isAdmin) values(@name,0,@image,@password,@address,@phone,@email,@acc,null,1)");
            }
        ...
        public void update(string account,string adminName, int gioiTinh, string image, string password, string address, string phone, string email)
        {
            ...
            if (gioiTinh == 1)
            {
                sqlCom = new MySqlCommand("update users set userName=@name,gioiTinh=1,urlAvatarPicture=@image,password=@password,address=@address,phone=@phone,email=@email where account=@acc");
            }
            else
            {
                sqlCom = new MySqlCommand("update users set userName=@name,gioiTinh=0,urlAvatarPicture=@image,password=@password,address=@address,phone=@phone,email=@email where account=@acc");
            }
            sqlCom.CommandType = CommandType.Text;
            sqlCom.Parameters.AddWithValue("@name", adminName);
            sqlCom.Parameters.AddWithValue("@password", BuildPassword(password));
        ...
        public void update(string account, string adminName, int gioiTinh, string image, string address, string phone, string email)
        {
            ...
            if (gioiTinh == 1)
            {
                sqlCom = new MySqlCommand("update users set userName=@name,gioiTinh=1,urlAvatarPicture=@image,address=@address,phone=@phone,email=@email where account=@acc");
            }
            else
            {
                sqlCom = new MySqlCommand("update users set userName=@name,gioiTinh=0,urlAvatarPicture=@image,address=@address,phone=@phone,email=@email where account=@acc");
            }
            sqlCom.CommandType = CommandType.Text;
            sqlCom.Parameters.AddWithValue("@name", adminName);
            sqlCom.Parameters.AddWithValue("@image", image);
        ...
        public void changePassword(string account, string password_new)
        {
            ...
            sqlCom = new MySqlCommand("update users set password=@password where account=@acc");
        ...
        {
            MySqlCommand sqlcom = new MySqlCommand("select * from users where account=@username and password=@pass and isAdmin=1");
            sqlcom.CommandType = CommandType.Text;
